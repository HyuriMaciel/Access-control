...
const char *faceCascadeFilename = "/home/hyurism/Dropbox/PIBITI_2015-16/facedec/HaarCascades/haarcascade_frontalface_alt.xml";
...
void recognizeFileList(const char *szFileTest)
{
	...
	if (nCorrect+nWrong > 0) {camera = cvCaptureFromCAM(0);
	//if (nCorrect+nWrong > 0) {camera = cvCaptureFromFile("http://admin:DavidFS@10.0.0.112/video2.mjpg");
...
// Will automatically initialize the camera on the first frame.camera = cvCaptureFromFile("http://admin:CLovisgabrielm@10.0.0.119/video2.mjpg");
...
IplImage* getCameraFrame(void)
{
	...
	if (!camera) {
		...
		//camera = cvCaptureFromFile("http://admin:DavidFS@10.0.0.112/video2.mjpg");
		//camera = cvCaptureFromFile("http://admin:facecloud@192.168.200.241/video2.mjpg");
		...
		cvSetCaptureProperty( camera, CV_CAP_PROP_FRAME_WIDTH, 400 );
		cvSetCaptureProperty( camera, CV_CAP_PROP_FRAME_HEIGHT, 300);
...
{
	int i;
	int contador=1, parada= -1;
	float confianca=0.970;
	...
	while (1)
	{
		...
		// Perform face detection on the input image, using the given Haar cascade classifier.
		faceRect = detectFaceInImage(greyImg, faceCascade, parada, arq1 );
...
// MAYBE GET IT TO ONLY TRAIN SOME IMAGES ?
		...
		if (faceRect.width > 0) {	// Check if a face was detected.
			...
			if (nEigens > 0) {	// Check if the face recognition database is loaded and a person was recognized.
				...
				if(confidence < confianca) {
				   snprintf(text, sizeof(text)-1, "Face desconhecida");
				   cvPutText(shownImg, text, cvPoint(faceRect.x, faceRect.y + faceRect.height + 15), &font, textColor);

				   //snprintf(nome, sizeof(nome)-1, "/home/davidfsilva/Desktop/imagens_teste/%d.pgm",contador );
				   //cvSaveImage(nome,shownImg);
				   //contador=contador+1;
                   if(parada >= 0){
				      fprintf(arq,"desconhecida\n");
                      }
				   }
				else{
